...
export const StarIcons = ({ className, size = 20, outline = false, strokeWidth = 2 }: StarIconProps) => {
  return <div className="flex gap-1">{Array.from({ length: 5 }).map((_, index) => (
    <StarIcon key={index} className={cn("text-amber-300", className)} size={size} outline={outline} strokeWidth={strokeWidth} />
  ))}
  </div>
};
